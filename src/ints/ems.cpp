...
static Bit16u
EMS_Defrag()                                                            // Defrag EMS memory and return free space in pages
{
    ...
    for (int i = 0; i < numHandles -
                        1; i++)                                            // Sort the handles in order of memory address
    ...
    for (int i = 0; i < numHandles; i++)                                            // Defrag compacting to start
    {
        ...
        if (handlePtr->addr > lowAddr) {
            ...
            for (int j = 0; j < EMS_MAX_PHYS; j++)                                    // Relocate page handles if needed
                if ((Bit32u) (pHandler[j].PageBase - MemBase) >= handlePtr->addr &&
                    (Bit32u) (pHandler[j].PageBase - MemBase) < handlePtr->addr + handlePtr->pages * EMS_PAGE_SIZE)
...
static Bit8u EMS_ReleaseMemory(Bit16u handle) {
    if (!ValidHandle(handle))                                                        // Check for valid handle
        return EMS_INVALID_HANDLE;
    if (handle !=
        0)                                                                // Not sure what to do with eventual refering page handle(s)
