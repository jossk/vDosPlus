...
Bit32u XMS_Defrag(Bit16u maxHandle)													// Defrag XMS memory, maximize specific handle (0 = n/a) and return free space in KB
	{
...
static Bit8u XMS_GetHandleInformation(Bit16u handle, Bit8u& lockCount, Bit16u& numFree, Bit32u& size)
	{
	if (InvalidXMSHandle(handle))
		return XMS_INVALID_HANDLE;
	numFree = 0;																	// Count available handles
	for (Bit16u i = 1; i < XMS_HANDLES; i++)
		if (xms_handles[i].free)
			numFree++;
...
static Bit8u XMS_ResizeMemory(Bit16u handle, Bit32u newSize)
	{
	if (InvalidXMSHandle(handle))
		return XMS_INVALID_HANDLE;	
...
Bitu XMS_Handler(void)
	...
	Bit16u cx=0;
	Bit32u res=0;
	if (TotXMSMB<16&&(reg_ah==0x88||reg_ah==0x89||reg_ah==0x8e||reg_ah==0x8f))
		reg_ah = 0x92;
	switch (reg_ah)
		{
	case XMS_GET_VERSION:
		reg_ax = TotXMSMB>15?0x300:0x200;											// XMS version 2.00/3.00
		reg_bx = TotXMSMB>15?0x301:0x201;											// Driver internal revision 2.01/3.01
	...
	case XMS_QUERY_A20:
		...
		break;
	case 0x88:
		reg_ecx = (64*MEM_PAGESIZE)-1;												// highest known physical memory address
	case XMS_QUERY_FREE_EXTENDED_MEMORY:
		reg_bl = XMS_QueryFreeMemory(res);
		if (reg_ah==0x88)
			{
			reg_eax = res;
			reg_edx = reg_eax;														// Free is always total
			}
		else
			{
			reg_ax=res>65535?65535:(Bit16u)res;
			reg_dx=reg_ax;
			}
	...
	case 0x89:
		if (reg_sp == 0x234 && Mem_Lodsw(SegPhys(ss)+reg_sp) == 0x5296)				// 4DOS signature
			...
			if ((_4DOS_Handles[index] = malloc(((Bit16u)(reg_ah==0x89?reg_edx:reg_dx))*1024)) == NULL)
				{
			...
			SET_RESULT(XMS_AllocateMemory(reg_ah==0x89?reg_edx:reg_dx, reg_dx));
		break;
	...
	case 0x8e:
		SET_RESULT(XMS_GetHandleInformation(reg_dx, reg_bh, cx, res), false);
		if (reg_ah==0x8e)
			{
			reg_edx=res;
			reg_cx=cx;
			...
			{
			reg_dx=res>65535?65535:(Bit16u)res;
			reg_bl=cx>255?255:(Bitu)cx;
			}
		break;
	case XMS_RESIZE_EXTENDED_MEMORY_BLOCK:
	case 0x8f:
		SET_RESULT(XMS_ResizeMemory(reg_dx, reg_ah==0x8f?reg_ebx:reg_bx));
		break;
