...
void Mouse_Moved(Bit16u x, Bit16u y, Bit16u scale_x, Bit16u scale_y)
	...
	if (abs(mouse.x-oldX)<16384)
		mouse.mickey_x+=mouse.x-oldX;
	if (mouse.mickey_x>32767)
		mouse.mickey_x=32767;
	else if (mouse.mickey_x<-37267)
		mouse.mickey_x=-32767;
	if (abs(mouse.y-oldY)<16384)
		mouse.mickey_y+=mouse.y-oldY;
	if (mouse.mickey_y>32767)
		mouse.mickey_y=32767;
	else if (mouse.mickey_y<-37267)
		mouse.mickey_y=-32767;
	if ((mouse.x-oldX)|(mouse.y-oldY))												// If at new position
...
void MOUSE_Init()
	...
	rcb = CALLBACK_RealPointer(cb);
	RealSetVec(0x33, rcb+(usesMouse ? 0 : 5));										// Programs should check for 0 (or IRET at location), 0 as Int address stalls some
