...
static Bitu INT10_Handler(void)
	...
	case 0x10:																		// Palette functions
		...
		case 0x03:																	// Toggle intensity/blinking bit
			if (blinking!=(reg_bl==1))
				{
				blinking=reg_bl==1;
				Mem_aStosb(BIOS_VDU_CONTROL, Mem_aLodsb(BIOS_VDU_CONTROL) + (blinking ?  0x20 : -0x20));
				}
			break;
...
graphics_chars:
	...
	case 0x4F:
		if (reg_al == 0x10)
			...
			else if (reg_bl == 1 && (reg_bh == 0 || reg_bh == 1 || reg_bh == 4))
				{
				::SendMessage(vDosHwnd, WM_SYSCOMMAND, SC_MONITORPOWER, reg_bh==0?-1:(reg_bh==1?1:2));
				if (reg_bh==0) mouse_event(MOUSEEVENTF_MOVE, 0, 0, 0, 0);
				reg_ah=0;
				reg_al=0x4F;
				}
			}
