...
static Bit8u flags1, flags2, flags3;
static Bit8u pastedData[
        txtMaxLins * txtMaxCols];                                        // About one screen full should be enough?
...
void
UpdateKBflags()                                                                // At least L.Shift + R.Shift are not captured by BIOS_addkey, so seperate
{
    Bit8u kbState[256];
    GetKeyboardState(kbState);

    flags1 = kbState[VK_RSHIFT]
            >> 7;                                                    // Flags are mostly masked by shifting
    flags1 |= kbState[VK_LSHIFT] >> 6;
    flags1 |= kbState[VK_CONTROL] >> 5;
    flags1 |= kbState[VK_MENU] >> 4;
    flags1 |= kbState[VK_SCROLL] << 4;
    flags1 |= kbState[VK_NUMLOCK] << 5;
    flags1 |= kbState[VK_CAPITAL] << 6;
    flags1 |= kbState[VK_INSERT] << 7;
    Mem_aStosb(BIOS_KEYBOARD_FLAGS1, flags1);

    flags2 = kbState[VK_LCONTROL] >> 7;
    flags2 |= kbState[VK_LMENU] >> 6;
    flags2 |= kbState[VK_SCROLL] >> 3;
    flags2 |= kbState[VK_NUMLOCK] >> 2;
    flags2 |= kbState[VK_CAPITAL] >> 1;
    flags2 |= (kbState[VK_INSERT] & 0x80);
    Mem_aStosb(BIOS_KEYBOARD_FLAGS2, flags2);

    flags3 = 0x10;                                                                    // Enhanced keyboard installed
    flags3 |= kbState[VK_RCONTROL] >> 5;
    flags3 |= kbState[VK_RMENU] >> 4;
    Mem_aStosb(BIOS_KEYBOARD_FLAGS3, flags3);

    Bit8u leds = (kbState[VK_SCROLL] & 0x01);
    leds |= kbState[VK_NUMLOCK] << 1;
    leds |= kbState[VK_CAPITAL] << 2;
...
//    vpLog("flag1:%02X flags2:%02X flags:%02X scancode:%02X unicode:%04X pressed:%d",
//          flags1, flags2, flags, scancode, unicode, pressed );
...
//		}
    ...
    if (scancode >= 0x47 && scancode <= 0x53) {                                       // Arrow keys/NumPad
        ...
        if (flags1 & 0x08)                                                            // Either Alt down
            return add_key(scan_to_scanascii[scancode].alt + 0x5000);
        else if (flags1 & 0x04)                                                        // Either Control down
            return add_key((scan_to_scanascii[scancode].control) | enhanced);
        else if ((flags1 & 0x3) || (flags1 & 0x20))                                        // Either Shift down or NumLock
            return add_key((scan_to_scanascii[scancode].shift & 0xff00) | enhanced);
        else
            return add_key((scan_to_scanascii[scancode].normal & 0xff00) | enhanced);
        return;
    }

    if (scancode <= MAX_SCAN_CODE) {                                 // Fallthru to report some combinations based upon the original IBM keyboard
        //vpLog("4");                                                                       // At least WP checks for instance for [Ctrl]6
        if (flags1 & 0x08)                                                            // Alt down
            return add_key(scan_to_scanascii[scancode].alt);
        else if (flags1 & 0x04)                                                        // Ctrl down
            return add_key(scan_to_scanascii[scancode].control);
        else if (flags1 & 0x03)                                                        // Either Shift down
            return add_key(scan_to_scanascii[scancode].shift);
        else
            return add_key(scan_to_scanascii[scancode].normal);                            // Last two shouldn't be needed, so what
    }
...
//            return add_key(0xe00d);
        if ((flags & 1)) {
            if (scancode == 0x1C) return add_key(0xe00d); // Keypad Enter
        }
...
//        else if ((flags1 & 0x3) ||
//                 (flags1 & 0x20))                                        // Either Shift down or NumLock
...
void Rudimentary_int09_handler(Bit8u scancode) {
    ...
    if ((flags2 & 0x03) ==
        2)                                                            // Left Alt down w/o Left Ctrl (=Right Alt)
    {
    ...
    if (scancode <=
        MAX_SCAN_CODE)                                                    // Fallthru to report some combinations based upon the original IBM keyboard
...
static bool IsEnhancedKey(Bit16u &
                          key)                                                // Check if key combination is enhanced or not, translate key if necessary
{
    if ((key >> 8) ==
        0xe0)                                                            // Test for special keys (return and slash on numblock)
    {
        if (((key & 0xff) == 0x0a) ||
            ((key & 0xff) == 0x0d))                            // Key is return on the numblock
            key = (key & 0xff) | 0x1c00;
        else                                                                        // Key is slash on the numblock
            key = (key & 0xff) | 0x3500;
        return false;                                                                // Both keys are not considered enhanced keys
    } else if (((key >> 8) > 0x84) || (((key & 0xff) == 0xf0) && (key
            >> 8)))                // Key is enhanced key (either scancode part>0x84 or specially-marked keyboard combination, low part==0xf0) */
        return true;
    if ((key >> 8) &&
        ((key & 0xff) == 0xe0))                                            // Convert key if necessary (extended keys)
        key &= 0xff00;
    return false;
}
...
static Bitu INT16_Handler(void) {
    ...
    switch (reg_ah) {
        case 0x00:                                                                        // Get keystroke
            if ((BIOS_GetKey(temp)) &&
                (!IsEnhancedKey(temp)))                            // Normal key found, return translated key in AX
            {
                VGA_ResetVertTimer(pastedHead != pastedTail && temp != 0x1c0d && pastedData[pastedHead + 1] !=
                                                                                 13); // Hide them till Enter (at least WP 6 is that sluggish, we can't hide till all)
                reg_ax = temp;
            } else {
        ...
        case 0x10:                                                                        // Get keystroke (enhanced keyboards only)
            if (BIOS_GetKey(temp)) {
                if (((temp & 0xff) == 0xf0) && (temp
                        >> 8))                                    // Special enhanced key, clear low part before returning key
                    temp &= 0xff00;
        ...
        case 0x01:                                                                        // Check for keystroke
            for (;;) {
                if (BIOS_CheckKey(temp)) {
                    if (!IsEnhancedKey(
                            temp))                                            // Normal key, return translated key in ax
                    {
                        reg_ax = temp;
                        CALLBACK_SZF(false);
                        break;
        ...
        case 0x11:                                                                        // Check for keystroke (enhanced keyboards only)
            if (!BIOS_CheckKey(temp)) {
                if (CPU_Cycles >
                    100)                                                    // Consume some cycles to allow irqs to happen
                    CPU_Cycles -= 100;
                CALLBACK_SZF(true);
            } else {
                CALLBACK_SZF(false);
...
void BIOS_SetupKeyboard(void) {
    ...
    CALLBACK_Install(0x16, &INT16_Handler,
                     CB_INT16);                                // Allocate/setup a callback for int 0x16
    if (keymode) {
        ...
        Bitu irq1_ret_ctrlbreak_callback = CALLBACK_Allocate();
        CALLBACK_Setup(irq1_ret_ctrlbreak_callback, &IRQ1_CtrlBreakAfterInt1B, CB_IRQ1_BREAK);
    }
}
