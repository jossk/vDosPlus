...
Bit8u FCB_Parsename(Bit16u seg, Bit16u offset, Bit8u parser, char *string, Bit8u *change) {
    ...
    Bit8u ret = 0;
    if (!(parser &
          PARSE_DFLT_DRIVE))                                               // Default drive forced, this intentionally invalidates an extended FCB
...
#pragma pack()
    fcb.GetName(
            fcb_name.full);                                                     // Get the old information from the previous fcb
...
bool FCB_OpenFile(Bit16u seg, Bit16u offset) {
    ...
    if (!DOS_MakeName(shortname, fullname,
                      &drive))                                 // First check if the name is correct
        return false;
...
static void SaveFindResult(DOS_FCB &find_fcb) {
    ...
    char file_name[9];
    char ext[4];
    find_dta.GetResult(-1, name, lname, size, date, time, attr);
    drive = find_fcb.GetDrive() + 1;

    DTAExtendName(name, file_name,
                  ext);                                            // Create a correct file and extention
    DOS_FCB fcb(RealSeg(dos.dta()), RealOff(dos.dta()));                            // TODO
    fcb.Create(find_fcb.Extended());
    fcb.SetName(drive, file_name, ext);
    fcb.SetAttr(
            attr);                                                              // Only adds attribute if fcb is extended
...
bool FCB_FindFirst(Bit16u seg, Bit16u offset) {
    ...
    fcb.GetAttr(
            attr);                                                              // Gets search attributes if extended
    bool ret = DOS_FindFirst(name, attr, -1);
    dos.dta(old_dta);
    if (ret)
        SaveFindResult(fcb);
    return ret;
...
Bit8u FCB_WriteFile(Bit16u seg, Bit16u offset, Bit16u recno) {
    ...
    date = DOS_PackDate(dos.date.year, dos.date.month,
                        dos.date.day);               // Ttime doesn't keep track of endofday
...
static Bit8u DOS_FCBIncreaseSize(Bit16u seg, Bit16u offset) {
    ...
    date = DOS_PackDate(dos.date.year, dos.date.month,
                        dos.date.day);               // Time doesn't keep track of endofday
    Bit32u ticks = Mem_Lodsd(BIOS_TIMER);
    Bit32u seconds = (ticks * 10) / 182;
    Bit16u hour = (Bit16u) (seconds / 3600);
    Bit16u min = (Bit16u) ((seconds % 3600) / 60);
 ...
 */
    ...
    fcb.GetRandom(
            random);                                                          // Set the correct record from the random data
...
Bit8u FCB_RandomWrite(Bit16u seg, Bit16u offset, Bit16u *numRec, bool restore) {
    ...
    if (*numRec > 0) {
        ...
        {
            error = FCB_WriteFile(seg, offset,
...
bool FCB_DeleteFile(Bit16u seg, Bit16u offset) {
    ...
    while (nextfile) {
        ...
        FCBNameToStr(shortname);
        bool res = DOS_DeleteFile(shortname, false);
        if (!return_value && res)
