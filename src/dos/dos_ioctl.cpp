...
bool DOS_IOCTL(void)
	...
	case 9:																			// Check if block device remote
		...
		char rootDir[] = " :\\";
		rootDir[0] = *Drives[drive]->basedir;
		if (reg_al == 8)
			reg_ax = rootDir[0] != '\\' && GetDriveType(rootDir)==DRIVE_REMOVABLE?0:1;	// Removable or fixed
		//else if (Drives[drive]->remote)
		else if (rootDir[0] == '\\' || GetDriveType(rootDir)==DRIVE_REMOTE)
			reg_dx = 0x1000;
		else
	...
	case 0x52:																		// DETERMINE DOS TYPE/GET DR DOS VERSION
		...
		return false;
	default:
		vpLog("DOS:IOCTL Call %2X unhandled", reg_al);
		DOS_SetError(DOSERR_FUNCTION_NUMBER_INVALID);
		break;
		}
	return false;
