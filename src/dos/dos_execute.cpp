...
static bool swapOut(Bit16u minsize)
	{
	Bit16u swapSize = 0;
	...
	if (lastSize  > minsize || lastSize+swapSize < minsize)							// If already enough space for 4DOS to swap into, or not enough after swapping by vDos
		return false;
	swapSize += 2;																	// Include last MCB free space
	...
	mcb.SetSize(lastSize+swapSize-35);
	return true;
	}
...
bool DOS_Execute(char * name, PhysPt block_pt, Bit8u flags)
	...
	if (Mem_Lodsb(comPtr)>126) Mem_Stosb(comPtr,126);
	Mem_CopyFrom(comPtr+1, cmdLine, Mem_Lodsb(comPtr));								// Get commandline
	if (strlen(cmdLine))
		vpLog("Execute: %s - %s", name, cmdLine);
	else
		vpLog("Execute: %s", name);
	fLen = strlen(name);
	is4DOS = 0;
		...
		if (fLen < sizeof(EXE_Header))
			...
			if (head.signature != MAGIC1)
				...
				if (head.e_lfanew < fPos-2)
					...
					if (winrun && fLen == 2 && fPos == head.e_lfanew && PEsign == 0x4550)
...
//								int ret = _spawnl(WinProgNowait, winName, winName, comline, NULL);
								int ret = _spawnl(P_NOWAIT, winName, qwinName, comline, NULL);
