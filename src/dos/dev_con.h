...
private:
    ...
    void Real_INT10_TeletypeOutputAttr(Bit8u chr, Bit8u attr, bool useattr) {
        ...
        Bit8u page = Mem_Lodsb((0x40 << 4) + 0x62);
...
bool device_CON::Read(Bit8u *data, Bit16u *size) {
    ...
    if ((readcache) && (*size)) {
        ...
        if (dos.echo)
            Real_INT10_TeletypeOutput(readcache, 7);
        readcache = 0;
    }
    while (*size > count) {
        ...
        switch (lowKey) {
            case 13:
                ...
                *size = count;
                if (dos.echo) {
                    Real_INT10_TeletypeOutput(13,
                                              7);                                        // Maybe don't do this (no need for it actually, but it's compatible)
        ...
        if (dos.echo)
            Real_INT10_TeletypeOutput(lowKey,
                                      7);                                        // what to do if *size==1 and character is BS ?????
    }
    *size = count;
    return true;
}
...
bool device_CON::Write(Bit8u *data, Bit16u *size) {
    ...
    while (*size > count) {
        if (!ansi.esc) {
            ...
            if ((data[count] == '\n') && (lastwrite != '\r'))
                Real_INT10_TeletypeOutputAttr('\r', ansi.attr, ansi.enabled);
        ...
        switch (data[count]) {
            ...
            case 'A':    // cursor up
                ...
                if (row < 0)
                    row = 0;
                Real_INT10_SetCursorPos(row, CURSOR_POS_COL(page), page);
                break;
            case 'B':    // cursor Down
                row = CURSOR_POS_ROW(page) + (ansi.data[0] == -1 ? 1 : ansi.data[0]);
                if (row >= ansi.nrows)
            ...
            case 'C':    // cursor forward
                ...
                if (col >= ansi.ncols)
                    col = ansi.ncols - 1;
                Real_INT10_SetCursorPos(CURSOR_POS_ROW(page), col, page);
                break;
            ...
            case 'f':        // Cursor Pos
                ...
                if (col > ansi.ncols)
                    col = (Bit8u) ansi.ncols;
                Real_INT10_SetCursorPos(--row, --col, page);    // ansi=1 based, int10 is 0 based
                break;
            ...
            case 'K':    // Erase in Line
                ...
                if (ansi.data[0] == -1 || ansi.data[0] == 0)    // Clear from cursor to the end of the line
                    INT10_WriteChar(' ', ansi.attr, page, ansi.ncols - col - 1,
                                    true);    // Extra -1 to prevent scrolling when end of screen is reached
                ...
                {
                    Real_INT10_SetCursorPos(row, 0, page);
                    INT10_WriteChar(' ', ansi.attr, page, ansi.ncols - 1,
                                    true);    // Extra -1 to prevent scrolling when end of screen is reached
                }
