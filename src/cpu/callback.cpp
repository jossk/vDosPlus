...
void CALLBACK_SetupExtra(Bitu callback, Bitu type, PhysPt physAddress) {
    switch (type) {
        case CB_RETF:
            Mem_aStosw(physAddress + 0,
                       0x38FE);                                            // GRP 4 + Extra Callback instruction
        ...
        case CB_IRQ1_BREAK:                                // return from int9, when Ctrl-Break is detected; invokes int 1b
            Mem_aStosw(physAddress + 0, 0x1bcd);        // int 1b
            Mem_aStosb(physAddress + 2, 0xfa);            // cli
            Mem_aStosw(physAddress + 3, 0x38FE);        // GRP 4 + Extra Callback instruction
            Mem_aStosw(physAddress + 5, callback);        // The immediate word
            Mem_aStosw(physAddress + 7, 0x20b0);        // mov al, 0x20
            Mem_aStosw(physAddress + 9, 0x20e6);        // out 0x20, al
            Mem_aStosb(physAddress + 11, 0x58);            // pop ax
            Mem_aStosb(physAddress + 12, 0xcf);            // An IRET Instruction
            break;
...
void CALLBACK_Init() {
    ...
    Mem_aStosb(CALLBACK_GetBase(),
               0xCF);                                            // Setup dummy (IRET) at first entry

    call_stop = CALLBACK_Allocate();                                                // Setup the Stop handler
    CallBack_Handlers[call_stop] = stop_handler;
    Mem_aStosw(CALLBACK_PhysPointer(call_stop) + 0,
               0x38FE);                            // GRP 4 + Extra callback instruction
